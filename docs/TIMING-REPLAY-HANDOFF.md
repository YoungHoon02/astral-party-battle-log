...
> **상태(2026-09-26):** S1·S2·S3 완료. 봇전 네 판을 기록했고 그중 세 판이 엄격 재생에서 일치했다(첫 판은 임시 end
> 도입 전이라 end가 없다). 건강 요약은 앞선 실게임에서, 후보 버퍼는 넷째 진단 게임에서 확인했다. 세부와 사용법은 11절.
> **표시 정책:** 계측 작업 자체는 표시 정책을 바꾸지 않았다. 다만 같은 브랜치에 그 기록 분석으로 찾은 별도 표시
> 규칙 수정(PK 창 열림으로 앞 대기 줄 공개, SIGNAL-GATING.md 1절 "창 열림")이 들어 있다.
...
- 넷째 판(`ScreenProbe`·`Replay`·`TraceTiming` 켬): 172,618건·16.2MB, `checkpoint`까지 엄격 재생 통과
  (결정 1,223건 일치). `[candidates]` 17건이 모두 `complete`로 보고됐다(늦은 주사위 16건, 짝 없는 신호 1건).
  게임 종료 처리가 호출되지 않아 이 판의 `[health]` 종료 요약은 남지 않았다. 후보 경로는 로컬 로그에만 보관하며,
  이 보고만으로 화면 시각이나 후보의 의미를 확정하지 않는다.
...
그대로라 가정 실험이 된다. 원래 자기 신호·창 닫힘으로 공개된 줄이 그 신호보다 먼저 나가면 "기존 신호 시각보다 빠른
줄"로 세어 검토 대상으로 보인다. 과거 신호가 실제 화면보다 늦었을 수 있어 조기 표시 판정이 아니며, 화면 정확도는
독립 증거가 있을 때만 판정한다. 엄격 재현 성공으로 표시하지 않는다.

### 남은 일

- 후보 보고를 실제 신호로 연결하려면 여러 판에서 같은 화면 사건과 대응하는지 별도로 검증한다.
- 화면 정확도 판정에 쓸 독립 증거(공개 시점 구간)를 기록에 넣는 방법. 지금은 해제 근거만 검증한다.
- 기록 켬 상태의 실게임 프레임 비용을 잰다(파일 크기는 약 2MB/분으로 확인).
