...
봇 행동이 연속될 때 게임 화면의 연출이 서버 메시지보다 뒤처지는 현상을 줄이기 위해,
오버레이가 사건을 순서대로 재생하며 표시 시점을 추정하도록 바꿨습니다. 내 입력에 대한
응답이 오면 대기 중인 줄을 표시하고 이후 시점을 다시 맞춥니다. 파일 로그는 수신 시각에
기록됩니다. **새 방식은 빌드만 통과했으며 게임 내 표시 시점과 PvP 동작은 검증 전입니다.**
