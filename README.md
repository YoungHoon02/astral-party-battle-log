...
게임 화면에서 이번에 발견한 줄의 변경 후 표시와 대체 폰트의 표시 폭은 아직 확인하지 못했습니다.

## v0.1.7 (사전 배포)

- 글자 크기를 바꾸면 로그창 너비도 함께 조절됩니다. 별도의 `Overlay.Width` 설정은 더 이상
  사용하지 않으며, 기존 설정 파일에 남은 값은 무시됩니다.
- 카드 기록을 껐을 때 결과 머리줄에 카드 이름이 드러나던 문제를 고쳤습니다.

게임 화면에서 긴 카드·PK 기록과 대체 폰트의 표시 폭은 아직 확인하지 못했습니다.
...
연출 길이는 AI전 전투 실측값을 기반 값을 유지합니다.
