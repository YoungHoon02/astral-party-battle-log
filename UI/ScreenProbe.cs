...
internal static class ScreenProbe
{
    ...
    public static bool Init(ManualLogSource log, Harmony harmony,
                            Action<ScreenSignal, int, bool, IntPtr> signal, Action onDisabled)
    {
        _log = log;
        try
        {
    ...
    public static void NoteActive(GameObject go, bool value)
    {
        ...
        try
        {
            ...
            if (!TryClassify(key, go, out (ScreenSignal Sig, int Pip)? tag) || tag is not { } t) return;
            if (Active.TryGetValue(key, out bool last) && last == value) return;
            Active[key] = value;
            _signal(t.Sig, t.Pip, value, key);
        }
        catch (Exception e)
        {
            Fail(e);
