...
- 창 크기는 `Width` × `Lines`(기본 460 × 14줄)로 고정됩니다. 로그 양에 따라 커지거나
  줄어들지 않으며, 창보다 긴 줄은 창 경계에서 잘립니다.
...
- 봇 행동이 이어질 때 오버레이가 연출 길이를 순서대로 계산해 결과가 화면보다 너무 일찍
  나타나는 문제를 줄였습니다. 사용자 결정 단계의 응답을 받으면 표시 시점을 다시 맞춥니다.
  파일 로그에는 수신 즉시 기록됩니다.
- 카드 기록을 꺼도 오버레이의 카드 연출 시간은 유지하고, 스킬 사용은 카드가 아닌 스킬
  연출 길이로 계산합니다.

순차 재생은 게임 없이 도는 하네스에서 검증했습니다. **실게임의 표시 시점과 PvP 동작은
아직 검증하지 않았습니다.** 연출 길이는 기존 실측 기반 값을 유지합니다.
...
| `Overlay.Width` | 460 | 창 가로 폭. 이보다 긴 줄은 잘립니다. 기존 설정값은 유지됩니다 |
