...
// 잔차가 음수면 화면이 모델보다 일찍 그 단계에 도달했다는 뜻이다 → 모델이 화면을 뒤늦게
// 좇고 있으니 그 구간의 줄들이 연출보다 먼저 나갈 위험이 커진다.
//
// **이 값으로 개별 줄의 조기 표시를 판정할 수는 없다.** 동기화는 구간 끝만 알려주므로,
// 사슬의 끝이 늦으면서 그 안의 모든 줄이 이를 수도 있다. 여기 수치는 그 대리 지표다.
//
...
console.log(`비대칭 계수 ${ASYM} (음수 = 구간이 모델보다 일찍 끝났다는 뜻, ${ASYM}배 가중)`);
...
// 음수는 "구간이 모델 예상보다 일찍 끝났다"는 뜻이다. 그 구간의 개별 줄이 각자
// 연출보다 먼저 떴는지는 이 값으로 알 수 없다.
console.log('\n구간별 잔차 (음수 = 구간 종료가 모델보다 이름)');
