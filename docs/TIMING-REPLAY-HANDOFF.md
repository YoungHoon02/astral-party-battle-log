...
> **상태(2026-09-26):** S1·S3 완료. 봇전 세 판을 기록했고 그중 두 판이 엄격 재생에서 일치했다(첫 판은 임시 end
> 도입 전이라 end가 없다). S2는 건강 요약만 실게임에서 확인했고, **후보 버퍼는 하네스만 통과해 완료 기준(8절 3단계
> 실게임 수집·보고 확인)을 아직 채우지 못했다.** 세부와 사용법은 11절.
...
- **S2 완료 기준:** `Diagnostics.ScreenProbe`를 켠 진단 게임에서 후보 수집 완전성과 `[candidates]` 보고를
  확인한다(8절 3단계 후반). 한 판에서 신호 발견을 보장하지 않으며, 후보가 없으면 없는 이유를 기록한다.
