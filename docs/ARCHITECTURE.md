...
화면에서도 캐릭터로 인식하므로 로그에서 쓸모가 없다. 이름표(`character`/`monster`)에서
이름을 못 찾으면 — 캐릭터 선택 전이라 `HeroId`가 비어 있거나, 이름표에 없는 id이면 —
**자리표시자**로 등록한다: 플레이어는 `{슬롯+1}P`(`2P`), 몹은 `몹`. `HeroId`가 확정되면
그때 바뀐다. 같은 캐릭터를 두 명이 고른 경우에만 뒤에 번호를 붙인다. 몹은 반대로 항상
번호를 붙인다 (`마법 찻주전자1`).

`Roster`가 `Player`에서 읽는 건 `Id/Slot/IsBot` 세 개, 그리고 이름표 키인
`Hero.HeroId`(필드 10 안의 필드 2) 하나뿐이다. **`Hero`는 손패(`Cards`, 필드 8)도
품고 있으므로 거기서 다른 필드를 읽는 코드를 추가하지 말 것.**

**`Nick`(필드 2)은 읽지 않는다.** 계정 닉네임이라 로그에 쓰면 공유할 때 남의 계정
정보가 같이 나간다. 예전에는 이름표가 없을 때의 폴백으로 썼는데, 그러면 선택 전 명단이나
이름표에 없는 캐릭터에서 닉네임이 표시명이 되고 재접속 캐시에까지 저장됐다. 표시에서
거르는 것이 아니라 **읽는 코드가 없게** 해 둔 것이므로 폴백으로 되살리지 말 것.

**`Player.Id`는 매치용 슬롯이 아니라 영구 계정 uid다.** 그래서 로그 본문에는 uid를
쓰지 않고 캐릭터 이름(없으면 자리표시자)만 쓴다. 디버깅용으로 `LogPlayerIds` 설정을
켜면 참가자 줄에만 uid가 붙는다.
...
- **캐릭터가 정해지기 전 명단은 참가자 줄로 찍지 않는다(`HasHero`).** 명단은 선택
  전후로 두 번 오는데, 첫 벌을 찍으면 같은 사람이 자리표시자와 캐릭터 이름으로 두 번 나온다.
- **이름이 같아도 `HeroId`가 바뀌면 갱신이다.** 이름표에 없는 캐릭터는 선택 뒤에도
  `2P` 그대로라 이름만 비교하면 변화가 없는 것으로 보여, 참가자 줄이 영영 안 찍힌다.
  이 경우 참가자 줄은 `· 참가자 2P`로 나온다.
...
- **맵이 새로 내보낸 몬스터는 등록(`MonsterRefreshS2C`)보다 첫 버프가 먼저 온다**
  ...
  모르는 대상이 든 `UpdateHeroAttr`은 대기열(`_deferred`)에 넣고 등록을 기다린다.
  예전에는 한 칸짜리여서, 모르는 대상이 든 결과가 연달아 오면 둘째가 첫째를 등록 전에
  밀어냈다.
  - 대기열은 **도착 순서대로** 앞에서만 꺼낸다. 뒤 항목이 먼저 풀려도 앞이 끝날 때까지
    기다린다 — 기한이 도착 순서대로 단조라 앞이 안 끝났으면 뒤도 안 끝났다.
  - 기한은 **`MonsterRefresh`를 뺀 수신 프레임 8개 또는 300ms 중 먼저 오는 쪽**이다
    (`DeferFrames`/`DeferWindow`). 등록은 같은 행동의 메시지 묶음 안에 따라오고 그 묶음은
    수 ms 안에 몰려 오므로(`SaidWindow` 참고) 300ms면 충분하다. 프레임 수는 여러 마리가
    한꺼번에 나와 모르는 결과가 연달아 오는 경우를 덮으면서, 붙들린 줄 뒤로 다른 줄이
    앞질러 나가는 폭과 **풀릴 때 차례 문맥이 바뀌어 있을 위험**을 작게 묶는 값이다.
    몬스터 등록은 기다리는 대상 자체라 세지 않는다. 둘 다 실측값이 아니라 고른 값이다.
  - 풀림과 기한은 `MonsterRefresh`/`RunningGame`/`StartGame`이면 **명단을 갱신한 뒤**,
    그 밖의 프레임이면 처리 전에 본다. 그래서 등록이 온 바로 그 프레임에 이름이 붙는다.
    프레임이 올 때만 보므로 기한이 지나도 다음 프레임(하트비트 포함)까지는 남아 있다.
  - 기한이 지나도 이름을 못 찾으면 예전처럼 `?id`로 확정해 기록한다. 줄을 버리지 않는다.
  - 붙들 때의 문맥(그룹·종류·연출 단위·PK 그룹)은 보존하지만, 오버레이 예약은 도착 순서로
    쌓이므로 붙들린 줄은 그사이 나온 줄 뒤에 보인다.
  - **판 경계(`Reset`)에서는 남은 항목을 버린다.** 지난 판 줄이 새 판 파일에 들어가면 안
    된다. 용량(16) 초과로 버릴 때와 함께 버린 수만 `Diag`에 남긴다 — 기한 안에서는 대기열이
    8을 넘지 않으므로 용량 초과는 불변식이 깨졌다는 뜻이다. 정상 해소와 기한 만료는 남기지 않는다.
  - **대기 여부(`Knows`)는 재접속 캐시를 보지 않고 이번 판 등록(`_entries`)만 본다.** 방이
    아직 확정되지 않은 사이(첫 `RunningGame`/`StartGame` 전)에는 캐시가 지난 방 것일 수
    있어서다. 캐시 이름은 대기열과 무관하게 `Name()`(`Restore`)에서 항상 되살아나므로,
    대기열 판정에서 캐시를 보태도 얻는 건 "한 프레임 먼저 나오는" 것뿐이고 방 확인 없이
    캐시를 믿는 새 경로만 하나 늘어난다.
  - **대기 판정은 효과 대상(필드 4의 `HeroAttrEffect.PlayerId`)만 본다.** 메시지 상단의
    행위자(필드 1)는 안 본다 — 단일 슬롯 시절부터의 범위다. 그래서 대상은 전부 등록됐는데
    행위자만 미등록이면 대기 없이 그 자리에서 `?id`로 나간다.
...
- **playerId를 원문으로 남기지 않는다.** 키는 id의 SHA-256 앞 8바이트이고, 조회할 때
  들어온 id를 같은 방식으로 해시해 맞춘다. 한 판에 수십 명이라 충돌은 사실상 없다.
- **해시는 익명화가 아니다.** 솔트가 없고 uid는 범위가 좁은 정수라, 후보 id를 차례로
  해시해 보면 되돌릴 수 있다. 원문 id가 눈에 띄거나 검색되지 않게 하는 정도다. 그래서
  **`roster-cache.tsv`는 공유하지 않는 파일이다** — 이슈에 첨부하거나 로그와 함께 보내지
  말 것. 해시 방식을 바꾸면 기존 파일과 호환이 끊기므로 바꿀 때는 헤더 버전을 같이 올린다.
- 값은 **이름표에서 찾은 캐릭터·몹 이름**과 슬롯·`HeroId`, 그리고 방 번호뿐이다.
  자리표시자(`2P`, `몹`)는 저장하지 않고, 닉네임은 애초에 읽지 않으므로 들어갈 수 없다.
- **헤더가 `v3`가 아니면 읽지 않고 지운다.** v2까지는 이름표가 없을 때 계정 닉네임이
  이름값으로 저장될 수 있었다. 헤더가 없거나 다른 파일도 같이 한 번 지우고, 다음 저장에서
  v3로 다시 만든다.
